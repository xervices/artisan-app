...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex h-[56px] w-full flex-row items-center justify-between gap-2 rounded-[4px] border border-input bg-white px-4 py-2 dark:active:bg-input/50 sm:h-9',
        Platform.select({
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      className={cn('px-2 py-2 font-cabinet text-xs text-muted-foreground sm:py-1.5', className)}
      {...props}
    />
  );
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      <SelectPrimitive.ItemText className="select-none font-cabinet-medium text-sm text-foreground group-active:text-accent-foreground" />
    </SelectPrimitive.Item>
  );
}
